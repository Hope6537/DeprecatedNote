			...
			while (true) {
				if (nT != n && s != 0) {
					if (x[ls = s < (n - nT) ? 0 : s - (n - nT)][ln = n <= (s + nT) ? n
							: nT + s][lm = mT] == 0) {
						q.add(new State(count + 1, ls, ln, lm)); // s돌n
				...
				if (mT != m && s != 0) {
					if (x[ls = s < (m - mT) ? 0 : s - (m - mT)][ln = nT][lm = (m - mT) < s ? m
							: mT + s] == 0) {
						q.add(new State(count + 1, ls, ln, lm)); // s돌m
						// System.out.print(ls+" "+ln+" "+lm+"\t");
					}
				}
				if (nT != 0 && s < 100) {
					if (x[ls = (nT + s) > 100 ? 100 : nT + s][ln = nT < (100 - s) ? 0
							: nT - (100 - s)][lm = mT] == 0) {
						q.add(new State(count + 1, ls, ln, lm)); // n돌s
						// System.out.print(ls+" "+ln+" "+lm+"\t");
					}
				}
				if (nT != 0 && mT != m) {
					if (x[ls = s][ln = nT < (m - mT) ? 0 : nT - (m - mT)][lm = nT < (m - mT) ? mT
							+ nT
							: m] == 0) {
						q.add(new State(count + 1, ls, ln, lm)); // n돌m
						// System.out.print(ls+" "+ln+" "+lm+"\t");
					}
				}
				if (mT != 0 && s < 100) {
					if (x[ls = (mT + s) > 100 ? 100 : mT + s][ln = nT][lm = mT < (100 - s) ? 0
							: mT - (100 - s)] == 0) {
						q.add(new State(count + 1, ls, ln, lm)); // m돌s
						// System.out.print(ls+" "+ln+" "+lm+"\t");
					}
				}
				if (mT != 0 && nT != n) {
					if (x[ls = s][ln = mT < (n - nT) ? mT + nT : n][lm = mT < (n - nT) ? 0
							: mT - (n - nT)] == 0) {
						q.add(new State(count + 1, ls, ln, lm)); // m돌n
						// System.out.print(ls+" "+ln+" "+lm+"\t");
					}
				}
				if (q.size() > 0) {
